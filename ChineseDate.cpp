...
int ChineseDate::YearDay() const
{
    ...
    /*************修正：考虑闰月的情况************/
    int leapMonth;
    leapMonth = ChineseCalendarDB::GetLeapMonth(m_year);
    if (leapMonth != 0 && m_month > leapMonth) {
        days += ChineseCalendarDB::GetMonthDays(m_year, m_month);
    }
    /*************修正：考虑闰月的情况************/
    days += m_day;
...
ChineseDate& ChineseDate::operator =(const QDate& d)
{
    m_year = d.year();
    m_month = d.month();
    m_day = d.day();
